...
<div class="card h-100 <?= isset($model->parent_id) ? "rounded-4 card-category" : 'border-0' ?>">
    <div class="card-body p-0 <?= isset($model->parent_id) ? "border-0" : '' ?>">
        <?php
            echo Html::a(
                ...
                ['view', 'id' => $model->id],
                ['class' => 'd-flex gap-2 px-2 py-1 h-100 ' . (isset($model->parent_id) ? 'justify-content-center' : '') . ' align-items-center link-dark text-decoration-none px-2', 'data-pjax' => 0]
            )
        ?>
